...
Site.prototype.findView = function( name ){
	//
	var search = new RegExp("/app/views/"+ name, "gi");
	var matches = [];
	// use file list to lookup the appropriate file
	for( var i in site._files ){
		var file = site._files[i];
		// remove the axtension of the file
	...
	// loop through matches
	// - first priority is the app folder
	var app = path.normalize( root +"app/views/"+ name );
	if( matches.indexOf( app ) > -1){
		return app;
	}
	var module = new RegExp(root +"node_modules/(.+)/app/views/"+ name, "gi");
	for( var j in matches ){
		var match = matches[j];
		if( module.test( match ) ){
			return match;
	...
	// - revert to the base folder
	var base = path.normalize( __dirname + '/../app/'+ name );
	if( matches.indexOf( base ) > -1){
		return base;
	}
