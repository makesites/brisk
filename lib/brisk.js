...
Site.prototype.init = function( modules, options ){
	...
	if( options.root ){
		root = options.root;
		this.config = this.loadConfig("brisk"); // triggers a config update
	...
	this._files = files.lookupDirs({ root: this.config.root });

	// load the support files
	this.helpers = files.loadDir("helpers");
	this.models = files.loadDir("models");
